...
- Attempts: two independent process restarts, `r1` and `r2`.
...
Vanilla returned the wrong answer on the original and changed-wording length-five tasks, reached the short-task destination, and took a wrong route on the letter-token task. Every correct answer is printed in [FLAG_CARD.md](FLAG_CARD.md).
...
Two of two flagged restarts arrived: `2 / 2 = 100%` for these two attempts. That locates this destination twice; it does not map reliability on future tasks.
