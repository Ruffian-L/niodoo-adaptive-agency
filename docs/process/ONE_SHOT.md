...
> **Historical route note (2026-08-16):** This page documents the hard-route runner,
> not the full current thesis. The completed Grok Seal, Jason & Sol Seal, recovered
> July event, and 56-memory durability result are synthesized in
> [`PAPER.md`](../../PAPER.md) and [`RECORD.md`](../../RECORD.md).
...
```bash
./run verify          # preferred — adds preflight, hash verification and a diff
cargo one-shot        # the bare harness alias
```

**On the two names.** `cargo one-shot` is an alias in `.cargo/config.toml` that runs
`src/bin/flag-route.rs`. The file `src/bin/one-shot.rs` is a recovered leftover; it ran
neither the flag nor the control, and it is not what the alias invokes. Model compute is
the pinned binary `2151c184…`; the vanilla control is pinned `llama-cli` `72c08ab8…`.
