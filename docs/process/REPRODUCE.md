...
> **Current entry point is `./run`.** The commands on this page still work, but they
> assume artifacts are already present and in the right place. `./run` checks the
> environment, fetches and hash-verifies everything first, and reports each assertion
> separately. Start at [`SETUP.md`](../../SETUP.md) unless you have a reason not to.

## One command: control first, then flag

```bash
./run verify          # preferred: preflight, hash checks, then the route, then a diff
cargo one-shot        # the underlying harness alias, no checks around it
```

`cargo one-shot` is an alias defined in `.cargo/config.toml`; it runs
`src/bin/flag-route.rs`. It is not the file `src/bin/one-shot.rs`, which is a recovered
leftover that ran neither the flag nor the control.

See [ONE_SHOT.md](ONE_SHOT.md) for path overrides and the fail-closed checks.

## Level 0: check the documentation contract

```bash
./run docs-check
```

This offline check aligns the canonical title, operational claim, seal names,
authorship, historical banners, internal links, and curated evidence hashes. It does
not rerun inference.
